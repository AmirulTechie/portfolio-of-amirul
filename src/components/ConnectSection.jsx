...
export default function ConnectSection() {
  ...
  const handleSubmit = async () => {
    ...
    setStatus("sending");
    // Replace this with your actual form submission (EmailJS, Resend, etc.)
  ...
  return (
    ...
    >
      {/* Section label */}
      <p className="text-white/30 text-[11px] tracking-[0.15em] uppercase mb-10 text-center">
        / CONNECT
      </p>
      ...
      <div className="w-full max-w-2xl bg-[#0f0f0f] border border-white/[0.08] rounded-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-[#0a0a0a]">
          ...
          <span className="text-white/25 text-[10px] tracking-[0.1em]">
            ≥_ AMIRUL@CONNECT:~
          </span>
          <div className="w-12" />
        </div>
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            {visibleLines.map((line, i) => (
              <p key={i} className="m-0 text-[13px] leading-relaxed">
                <span className="text-white/30">guest@system:~$</span>{" "}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 flex flex-col gap-1">
                <div className="flex items-center gap-2 border-b border-white/10 pb-2 focus-within:border-white/30 transition-colors duration-200">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-white/70 text-[11px] tracking-[0.08em] placeholder:text-white/20 outline-none border-none"
                  />
                </div>
              </div>
              <div className="flex-1 flex flex-col gap-1">
            ...
            {/* Message */}
            <div className="flex items-start gap-2 border-b border-white/10 pb-2 focus-within:border-white/30 transition-colors duration-200">
              <span className="text-white/30 text-[11px] mt-0.5">&gt;</span>
              <textarea
                placeholder="YOUR_MESSAGE"
                rows={4}
                value={formData.message}
                onChange={(e) => setFormData({ ...formData, message: e.target.value })}
                className="flex-1 bg-transparent text-white/70 text-[11px] tracking-[0.08em] placeholder:text-white/20 outline-none border-none resize-none leading-relaxed"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-6">
            <a
              ...
              className="flex items-center gap-2 text-white/40 hover:text-white/70 text-[11px] tracking-[0.06em] no-underline transition-colors duration-200"
            >
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-2 border border-white/25 hover:border-white/60 hover:bg-white/5 text-white text-[11px] tracking-[0.12em] uppercase px-5 py-2.5 transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {status === "sending" ? (
                "TRANSMITTING..."
              ) : status === "sent" ? (
